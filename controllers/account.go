...
func (this *AccountController) Post() {
	...
	if len(uid) == 0 {
		...
		if len(pn) != 0 && len(eps) != 0 {
			...
			id, token := models.BH_Login("", "", 5, params)
			if len(id) != 0 {
...
<body>
    ...
    <p>Someone has register an account on BiHuHelper, confirm it or ignore.</p>
    <a href="http://bihuhelper.tk/account/confirmemail?uname={{.Name}}&hash={{.Hash}}">Click to Confirm</a>
</body>
</html>
`
	...
	err := send(to, "BiHuHelper: Account Confirmation", body.String(), "html")
	if err != nil {
		beego.Error(err)
	}
}
